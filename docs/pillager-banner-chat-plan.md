...
**関連:** [bug-player-chat-observation-gaps.md](bug-player-chat-observation-gaps.md)、[sound-identity-plan.md](sound-identity-plan.md)、[event-schema.md](event-schema.md)、[dialogue-design.md](dialogue-design.md)、[senryu-rag-plan.md](senryu-rag-plan.md)
...
| 経路 | してよいこと | してはいけないこと |
|---|---|---|
| **音** | 乗った音・解決できた種名だけ | 空 hearing から種名を足す（[sound-identity-plan](sound-identity-plan.md)） |
| **視界（載ってる / バッファ）** | 「おる／ついさっきおった」 | 無い type を見たことにする |
| **視界なし + プレイヤーが指している** | 「俺には見えんけど、それなら **カタログ候補** かもしれん」 | 「おらへん」「気のせい」と**否定して落とす** |
| **知識（④）** | biome×structure で「ありうる／出にくい」 | カタログ外の捏造 |
...
| 場所 | 役割 |
|---|---|
| `entry_catalog.py` | `find_catalog_topics(text, *, observed_ids=()) -> list[TopicHit]` |
| player_chat 組み立て | `catalog_topic_hints` を details へ |
| `prompts.py` | **§2.5b を規則の正本として載せる** |
| カタログ | 通称不足分を poetic 等に追加（aliases フィールドは任意） |
...
- [ ] 1 関数で旗・ババア・オッサン等が同じ経路  
- [ ] 視認ありなら該当 type が上がりやすい  
- [ ] 無関係文で hints なし  
- [ ] 新通称は **JSON 追加だけ**で伸びる  
...
**プロンプトの観測・否定・音・語彙規則の正本は §2.5b。** ここは実装チェックリスト。

| 項目 | 内容 | PR |
|---|---|---|
| threat_summary | 視認行 + バッファ「ついさっき」+ 任意 equipment | C, D, E |
| tactics | visual + バッファの type で合成。**トピック候補は観測があるときだけ** tactics に混ぜる | E |
| プロンプト | §2.5b を反映。ヒント外種族へのすり替え禁止 | E（骨子は B でも可） |
| fallback | 「おう、聞こえとるで〜。」廃止 → 中立「ようわからん、もうちょい教えて」 | **A で最低限**、E で磨き |
| unusable | 英字 NPC 等でも中身のある日本語 fallback | A / E |

完了条件:

- [ ] 視認 + 話題一致 → 種名・特徴が自然に出る  
- [ ] 視認なし + トピック → 「見えんけど〜かもしれん」  
- [ ] プレイヤーの「いる」を非検出だけで否定しない  
- [ ] 「聞こえとる」系に飛ばない  
...
プロンプト: **生成しうる** と **いま視界にある** を混同しない。

### 4.4 完了条件

- [ ] 主要 structure に `related_mobs`  
- [ ] details に plausibility 行  
- [ ] タイガ + 前哨？ → ありうる系  
- [ ] 深海以外 + 海底神殿？ → 出にくいかも系  
- [ ] ピリいる＝前哨確定にしない  
...
| **PR-D** | ①-B/C adapter（実測後） | ログ |
| **PR-E** | ③ tactics / プロンプト仕上げ / fallback 磨き | B, C,（F あれば尚良） |

推奨体感順: **A → B → C**。前哨会話は **B → F**。LOS が酷いなら **D 前倒し**。

受け入れ:

- **B:** 旗・ババア・オッサンが同一 API。キーワード表なし  
- **F:** taiga+前哨で「ありうる」。ピリ視認のみで前哨確定しない  
- **E:** §2.5b がプロンプトに反映済み  
...
3. 視界ギャップでもプレイヤー観測を否定して落とさない  
4. 音の「空から種名を足さない」と経路分離で両立  
5. poetic / 通称タグが検索と描写の両方に効く  
6. 新話題は **カタログ更新**で伸びる（コード分岐を増やさない）  
7. structure×biome で「ありうる／出にくい」をカタログ根拠で言える  
...
| プロンプト規則 | 2.5c と 3.4 で重複 | **正本は §2.5b。③はチェックリスト** |
...
| PR-E と fallback | A と E の両方 | **A=最低限、E=磨き** |
...
1. **PR-A + PR-B** から進めるか  
2. 続けて **PR-F**（structure リンク）か  
3. それとも snapshot 実測 → **PR-D** を先にするか  
