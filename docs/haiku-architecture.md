...
7. 合格案も未保存の `pending_revision` に置く。自然な採否はpending専用OS AI schemaが閉じたactionと発話中evidenceへ抽出し、コードが同じ差分を同じ元句へ適用できるか再確認してから保存する
...
プレイヤー自身が置換語を述べた場合、OS AIは句を生成せず、発話中の `replacement_text` / `evidence` と現在句中の `target_fragment` だけを抽出する。コードが発話根拠・対象行の一意性・ひらがな化・正確な5/7/5音・hard制約・CASを確認し、合格した三行だけを未保存案として提示する。
