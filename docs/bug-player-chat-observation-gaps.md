...
- **2026-08-13 乗り物を実装:** `player.vehicle` は乗車中だけ送る。未乗車時はキーを省略する
- 船・いかだはパドル、馬系は操縦者＋方向入力＋実移動、ラクダはダッシュをコード判定する
- 雑談と川柳には内部状態ではなく、`プレイヤーはXXに乗って走っている` のような主語付き事実だけを渡す
- **保留:** エリトラ滑空は乗り物ではない。`player_activity` を一般化するときの別課題とする
