...
def build_haiku_messages(details: dict[str, object]) -> list[dict[str, str]]:
    ...
    user_prompt = (
        ...
        "以下の候補からランダムに2つ選び、その2つだけをモチーフにすること。\n"
        f"{candidate_lines}\n"
        "\n"
        "コード側で見つけた状況の取り合わせ候補:\n"
        f"{tension_lines}\n"
...
def build_haiku_irony_messages(details: dict[str, object]) -> list[dict[str, str]]:
    ...
    peaceful_mobs = "、".join(str(item) for item in details.get("peaceful_mobs", []) if item) or "なし"
    user_prompt = (
        "以下の Minecraft 状況から、川柳のネタになる『状況の皮肉・矛盾・意外な取り合わせ』を1つだけ選ぶ。\n"
        "弱ければ found=false にする。\n"
        "必ず JSON オブジェクトだけを返す。\n"
        "説明文や markdown は禁止。\n"
        "\n"
        "返答形式:\n"
        "{\"found\": true/false, \"kind\": \"contrast|irony|juxtaposition|scene\", "
        ...
        f"- 詩語ヒント: {haiku_tags}\n"
        "\n"
        "おもしろさが弱ければ found=false を返す。"
    )
    return [
        {
            "role": "system",
            "content": (
                "あなたはMinecraftの状況から、川柳に向いた皮肉や矛盾だけを抽出する。"
                "返答は JSON オブジェクトのみ。"
            ),
        },
        {"role": "user", "content": user_prompt},
